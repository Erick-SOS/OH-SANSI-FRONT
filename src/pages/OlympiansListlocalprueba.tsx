...
/* ---------- Validaciones de texto alfa (máx 100, sin números/especiales) ---------- */
/** Acepta letras (incluye tildes y ñ) y espacios, máx 100 chars. */
...
export const OlympiansListLocal: React.FC = () => {
  ...
  const downloadTemplate = () => {
    const template =
      "numero,nombreCompleto,ci,unidadEducativa,modalidad,departamento,areaDeCompetencia,nivel,tutorLegal\n" +
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    // Solo CSV: si no, modal especial
    const isCSV =
      file.name.toLowerCase().endsWith(".csv") ||
      file.type === "text/csv" ||
      file.type === "application/vnd.ms-excel";
    if (!isCSV) {
      setOpenWrongType(true);
      return;
    }
    ...
    if (matrix.length === 0) {
      setImportErr("El archivo está vacío.");
      return;
    }
    ...
    if (missing.length) {
      setImportErr(
        `Faltan columnas obligatorias. Usa estos encabezados (en cualquier orden): numero,nombreCompleto,ci,unidadEducativa,modalidad,departamento,areaDeCompetencia,nivel,tutorLegal`
      );
      return;
    }
    ...
    const pushErr = (fila: number, columna: string, motivo: string, valor?: string) =>
      errors.push({ fila, columna, motivo, valor });
    ...
    for (let i = 1; i < matrix.length; i++) {
      ...
      // validaciones de alfa100
      const vNombre = validateAlpha100("Nombre completo", nombre);
      if (vNombre) pushErr(i + 1, "nombreCompleto", vNombre, nombre);
      if (unidad) {
        const vUnidad = validateAlpha100("Unidad educativa", unidad);
        if (vUnidad) pushErr(i + 1, "unidadEducativa", vUnidad, unidad);
      }
      const vMod = validateAlpha100("Modalidad", modalidadRaw);
      if (vMod) pushErr(i + 1, "modalidad", vMod, modalidadRaw);
      const vDep = validateAlpha100("Departamento", dep);
      if (vDep) pushErr(i + 1, "departamento", vDep, dep);
      const vArea = validateAlpha100("Área de competencia", area);
      if (vArea) pushErr(i + 1, "areaDeCompetencia", vArea, area);
      const vNivel = validateAlpha100("Nivel", nivel);
      if (vNivel) pushErr(i + 1, "nivel", vNivel, nivel);

      // modalidad válida (opcional, si quieres estrictamente dos valores)
      const modalidad: Modality =
        modalidadRaw.toLowerCase() === "grupal" ? "Grupal" : "Individual";

      // si hubo errores en la fila, no la agregamos
    ...
    if (!imported.length && errors.length) {
      ...
      setSummary({ ok: 0, fail: errors.length }); // una fila puede tener múltiples errores; aquí contamos errores totales
    ...
    for (const r of imported) {
      if (numerosTomados.has(r.numero)) r.numero = ++maxNumero;
      numerosTomados.add(r.numero);
    }
    ...
    <div className="p-6">
      ...
      <CenterModal open={openImport} onClose={() => setOpenImport(false)}>
        <div className="mb-4 flex items-center justify-between">
          ...
          <div className="flex gap-2">
            <button className="rounded-md border px-3 py-1.5 text-sm" onClick={downloadTemplate}>
              Descargar plantilla
            </button>
            <button className="rounded-md border px-3 py-1.5 text-sm" onClick={() => setOpenImport(false)}>
              Cerrar
            </button>
          </div>
        </div>
        ...
        <div
          ...
          onDrop={onDrop}
          className={`flex min-h-[220px] items-center justify-center rounded-xl border-2 border-dashed ${
        ...
        >
          <div className="text-center">
            ...
            <label className="mt-3 inline-block cursor-pointer text-sm text-indigo-700 underline">
              Buscar Archivo
              <input type="file" accept=".csv" className="hidden" onChange={onInputChange} />
            </label>

            <div className="mt-4 text-xs text-gray-500">
              Encabezados: <code>numero,nombreCompleto,ci,unidadEducativa,modalidad,departamento,areaDeCompetencia,nivel,tutorLegal</code>
              <br />
              Reglas: campos de texto deben contener **solo letras y espacios**, máx **100** caracteres.
            </div>
          </div>
        </div>
      </CenterModal>
