...
const datosIniciales: AreaEstado[] = [
  { id: 1, nombre: 'Matemáticas', codigo: 'MAT', descripcion: 'Razonamiento lógico, álgebra, geometría y estadística', area: 'Matemáticas' },
  { id: 2, nombre: 'Lenguaje y Literatura', codigo: 'LEN', descripcion: 'Comprensión lectora, redacción, gramática y análisis literario', area: 'Lenguaje y Literatura' },
  { id: 3, nombre: 'Física', codigo: 'FIS', descripcion: 'Mecánica, termodinámica, electromagnetismo y óptica', area: 'Física' },
  { id: 4, nombre: 'Química', codigo: 'QUI', descripcion: 'Reacciones químicas, enlaces, estequiometría y orgánica', area: 'Química' },
  { id: 5, nombre: 'Biología', codigo: 'BIO', descripcion: 'Células, genética, evolución y sistemas biológicos', area: 'Biología' },
  { id: 6, nombre: 'Geografía', codigo: 'GEO', descripcion: 'Geografía física, humana, cartografía y recursos naturales', area: 'Geografía' },
  { id: 7, nombre: 'Historia', codigo: 'HIS', descripcion: 'Historia universal, nacional y procesos sociales', area: 'Historia' },
  { id: 8, nombre: 'Informática', codigo: 'INF', descripcion: 'Algoritmos, programación y pensamiento computacional', area: 'Informática' },
];
...
const Areas: React.FC = () => {
  ...
  const handleOrdenar = (columna: string, direccion: 'asc' | 'desc') => {
    ...
    const sorted = [...datosAreas].sort((a, b) => {
      const valA = a[columna as keyof AreaEstado];
      const valB = b[columna as keyof AreaEstado];
  ...
  const confirmarAgregar = (formData: { nombre: string; codigo: string; descripcion: string }) => {
    const nuevoId = Math.max(...datosAreas.map(a => a.id), 0) + 1;
    const nuevaArea: AreaEstado = {
      id: nuevoId,
    ...
    setDatosAreas(prev => [...prev, nuevaArea]);
    cerrarModal();
  };

  const columnas = [
    { clave: 'area' as const, titulo: 'Área', alineacion: 'izquierda' as const, ordenable: true },
  ...
  const renderAcciones = (fila: AreaEstado) => (
    <div className="flex justify-center gap-2">
      ...
      <button onClick={() => handleEliminarArea(fila.id, fila.nombre)} className="text-red-600 dark:text-red-400 hover:text-red-800 dark:hover:text-red-300">
        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div className="flex-1 max-w-md">
              <BarraBusquedaAreas
                ...
                onBuscarChange={(t) => { setBusquedaAreas(t); setPaginaAreas(1); }}
              />
            </div>
            <button
              onClick={abrirModal}
