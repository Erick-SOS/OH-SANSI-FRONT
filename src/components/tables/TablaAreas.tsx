// src/components/tables/TablaAreas.tsx
import React, { useState, useMemo } from 'react';
import TablaBase from './TablaBase';
import { Area } from '../../api/areas';

type AreaConAlias = Area & {
  area: string;
  numero: number;
};

interface TablaAreasProps {
  datos: Area[];
...
const TablaAreas: React.FC<TablaAreasProps> = ({
  datos,
  onEliminarFila,
  paginaActual,
  registrosPorPagina,
  ...
  const datosParaTabla: AreaConAlias[] = datosOrdenados.map((item, index) => ({
    ...item,
    area: item.nombre || '-',
  ...
  const columnas = [
    {
      clave: 'area' as keyof AreaConAlias,
      titulo: 'Área',
      alineacion: 'izquierda' as const,
      ordenable: true,
    },
    {
      clave: 'codigo' as keyof AreaConAlias,
      titulo: 'Código de área',
      alineacion: 'izquierda' as const,
      ordenable: true,
    },
    {
      clave: 'descripcion' as keyof AreaConAlias,
      titulo: 'Descripción',
      alineacion: 'izquierda' as const,
      ordenable: true,
    },
  ...
  const renderAcciones = (fila: AreaConAlias) => (
    <button
      onClick={() => fila.id && onEliminarFila(fila.id, fila.area)}
